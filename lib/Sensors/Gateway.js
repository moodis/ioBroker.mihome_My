...
function Gateway(sid, ip, hub, model) {
   ...
   this.OnOffCfg     = undefined;
    ...
    if (this.hub.protoMajor(this.ip) === '2')  {
        this.hub.sendMessage({cmd: 'discovery', sid: this.sid}, this.ip);
        this.hub.emit('warning', 'sid : ' + sid + ' ip : ' + this.ip);       //  ?
    } else {
        this.hub.sendMessage({cmd: 'get_id_list', sid: this.sid}, this.ip);     //  ?
        this.hub.emit('warning', 'sid : ' + sid + ' ip : ' + this.ip);       //  ?
    }
...
Gateway.prototype.getData = function (data) {
    ...
    if (data.illumination !== undefined) {
	    ...
	    // this.illumination = data.illumination;
      	obj.illumination  = String(this.illumination);
        newData           = true;
    }
